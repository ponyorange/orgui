...
1、跳转到详情页，返回的时候还在原来的位置。

2、多个页面的表单输入，在页面间切换时输入内容不变。

3、单页面应用要实现微信小程序的路由效果：微信小程序在不同tab之间的切换是不会销毁页面的，navigate到新页面时，旧页面也不会被销毁，返回的时候还是原来那个页面。
